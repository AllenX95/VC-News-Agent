...
## 10. HTML 展示契约与 Codex 自由区

日报采用“稳定展示外壳 + 结构化内容自由区”的长期原则，当前 `daily_v1.html.j2` 的视觉方向作为基线保留。

稳定展示外壳由应用模板负责：

- 固定技术进展、产业新闻、融资新闻三个一级类目；
- 每条新闻使用全宽长条，不使用多列卡片或瀑布流；
- 信息层级固定为“主标题 → 简介 → source/时间/标签”；
- 保留 `data-report-item`、content/event ID、原始 URL 和三个 section key；
- 保持 Apple 式克制排版、轻分隔线、移动端适配、打印和离线打开能力；
- 日常 Automation 不生成或修改 HTML/CSS。

Codex 或日级报告 LLM 可以在经过 schema、ID 白名单和 URL 追溯校验后自由处理：

- 日报 headline 与 executive summary；
- 二级主题名称、主题顺序和条目排序；
- 条目入选、跨来源语义去重和分类取舍；
- 简介、投资意义和非事实性标签；
- 在三个固定一级类目内部，根据当日内容调整信息密度。

Codex 的自由不得改变持久化来源 URL、source、content/event ID，不得让同一新闻跨一级类目重复出现，也不得绕过 `report_data` 直接拼接展示代码。视觉方向需要调整时，应作为模板版本变更单独开发、预览和验收，而不是由每日任务临时发挥。
